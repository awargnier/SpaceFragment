...
fragment = [
    Fragment(id="s1", fragment="Moon", price="30€"),
    Fragment(id="s2", fragment="Mars", price="50€"),
    Fragment(id="s3", fragment="Meteorite", price="75€")
]
...
async def create_fragment(givenFragment:FragmentNoID, userData: int = Depends(get_current_user)):
    ...
    newFragment = Fragment(id=str(generatedId), fragment=givenFragment.fragment, price=givenFragment.price)
...
async def get_fragment_by_ID(fragment_id: uuid.UUID, userData: int = Depends(get_current_user)):
    fireBaseobject = db.child('fragments').child(userData['uid']).child('fragments').child(str(fragment_id)).get(userData['idToken']).val()
...
async def modify_fragment_name(fragment_id:str, modifiedFragment: FragmentNoID, userData: int = Depends(get_current_user)):
    fireBaseobject = db.child("fragments").child(userData['uid']).child('fragments').child(fragment_id).get(userData['idToken']).val()
    if fireBaseobject is not None:
        updatedFragment = Fragment(id=fragment_id, **modifiedFragment.model_dump())
        return db.child("fragments").child(userData['uid']).child('fragments').child(fragment_id).update(updatedFragment.model_dump(), userData['idToken'] )
    raise HTTPException(status_code= 404, detail="Fragment not found")
...
async def delete_fragment(fragment_id:str, userData: int = Depends(get_current_user)):
    ...
    if fireBaseobject is not None:
        return db.child("fragments").child(userData['uid']).child('fragments').child(fragment_id).remove(userData['idToken'])
    raise HTTPException(status_code= 404, detail="Fragment not found")
